...
with DAG('pipeline', default_args=default_args, schedule_interval=None) as dag:
    ...
    call_trigger = TriggerDagRunOperator(
        task_id='upload_storage_task_trigger',
        trigger_dag_id= 'dag_trigger',
        trigger_run_id='pipeline_trigger',
        execution_date=None,
        reset_dag_run=False,
        wait_for_completion=False,
        allowed_states=["Stream","started"],
        failed_states=["Streaming", "query", "has", "been", "idle", "and", "waiting", "for", "new", "data"]
    )


deploy_kafka_resource >> [kafka_streaming_task, spark_messaging_task >> call_trigger ]
